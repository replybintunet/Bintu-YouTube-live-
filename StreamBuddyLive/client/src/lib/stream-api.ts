...
export const streamApi = {
  ...
  startStream: async (
    ...
    filePath: string,
    loop: boolean = true,
    streamMode: 'desktop' | 'mobile' = 'desktop'
  ) => {
  ...
  /** ✅ FIXED: Pass streamId and hit the correct endpoint */
  getStreamStatus: async (streamId: number): Promise<StreamStatus> => {
    const response = await apiRequest('GET', `/api/stream/${streamId}/status`);
    return response.json();
  },
